...
if ($project -match '(?m)^\s*DEVELOPMENT_TEAM\s*=' -or
    [regex]::Matches($project, [regex]::Escape('MARKETING_VERSION = 0.4.1;')).Count -ne 2 -or
...
Write-Host "Validated manual protected TestFlight INTERNAL-ONLY workflow for Tarot Deck 0.4.1 (1): main-only cloud signing is parameterized, only non-binary evidence is preserved, and no external/App Store path exists."
