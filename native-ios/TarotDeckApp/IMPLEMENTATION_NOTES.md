...
A-056 is governed by V-107. One Card remains the first selector tile, Three Cards remains second and is the clean-install default, Six Cards uses exactly six miniatures in a 2x3 glyph, and Custom retains its dedicated outlined-layout plus glyph. V-107 replaces the erroneous V-090 without migrating or overwriting a saved choice or an active restored reading.

The live Read flow follows the approved Ceremonial Obsidian references through V-107. V-107 governs the reading-kind selector and V-081 the neutral five-style grid. V-097/V-098 govern shuffled and partial manual placement in portrait; V-100 governs tracked-top shuffle and placement motion; V-101/V-102 cover dense six-card and custom partial tables. V-103–V-105 replace the earlier landscape rail with a compact header, position labels above one shared card rectangle, a large explicit deck and one short cue. V-106 adds the bilingual portrait orientation recommendation for multi-card readings. V-086/V-087 continue to govern the complete table with no deck, and V-088 governs contextual tutorial navigation. V-064 still governs only the stable dark translucent primary tab bar material. Learn Index and Reading Tutorials follow V-072/V-073; V-071 governs the concise three-section article template. Card detail uses the plain `Meaning / Significado` heading, while `In a reading / En una tirada` applies that meaning to the question and assigned position.
...
- `Screens/Read/ReadViews.swift`: responsive Read Home, reading choices and generic 1–12-card table. Empty Home keeps its normal-size non-scrolling V-044 composition, uses a compact fitting variant on small iPhones, and permits scrolling only for accessibility Dynamic Type while the Settings gear remains overlaid. Reading Table derives every empty/back/front/flip/overlay surface from one post-clamp slot geometry with at least 14 points of visible separation. If valid custom points remain too close even at the minimum authored-layout width, a deterministic row-major presentation grid separates only their on-screen centers; the saved custom points and order are unchanged. Landscape has no redundant rail or status copy, while portrait multi-card readings show a discreet rotation recommendation. The portrait header uses a minimum height plus Dynamic Type-aware vertical padding inside the existing accessibility scroll path instead of clipping to a fixed height.
...
Shuffle creates the real persisted session. Manual placement, reveal and conceal each use an independent coordinator command and refresh from the coordinator only after its persistence boundary succeeds. A placement writes the next deck card and chosen slot in the same atomic session replacement, so interruption cannot separate them. Navigation among Read, Learn and Cards does not replace the model. Ending or replacing a reading clears the coordinator and continuity record through the approved flow.
...
V-100 and `design/tarot-deck/MOTION_SPEC.md` define the live motion language. Each valid pre-placement deck tap commits a fresh full order, then presents press, split/cut, old-top burial, interleave, riffle, a visibly different top layer and square. Tapping any empty position commits the next deck card and that exact `positionIndex` in one atomic session write before the card-back travels to the chosen slot. Shuffle becomes unavailable after the first placement; the deck stays as a static physical source until the final slot is occupied, then disappears. Reveal/conceal use independent two-surface vertical-axis turns whose destination face remains hidden until the midpoint; reset is the only quick restart.

Motion observes only the already-published `DeckSession` signature, so a failed save cannot trigger a success haptic or animation. Presentation keeps its own cancellable token and slot-indexed visual baseline; the success haptic fires only after settle, landing or flip completion while the scene remains active. Restoration, rotation, backgrounding or leaving the table cancels overlays and establishes the latest committed state without replay. Reduce Motion and VoiceOver replace displacement, scale and 3D turns with short opacity changes; decorative packets and placement overlays remain hidden from accessibility while each committed slot supplies semantics. The implementation uses only SwiftUI/UIKit APIs available on iOS 16 and contains no looping ambience, particles or iOS 17 motion APIs.
