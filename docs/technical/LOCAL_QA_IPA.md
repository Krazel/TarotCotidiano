...
- `TarotDeck-<version>-<build>-local-qa-unsigned.ipa`;
...
2. In PowerShell, run:
   ...
   ```powershell
   Get-FileHash .\TarotDeck-0.0.1-1-local-qa-unsigned.ipa -Algorithm SHA256
   ```

3. Compare the reported hash with the first value in the adjacent `.sha256` file.
4. Open the `.manifest.json` and confirm `configuration` is `Debug`, `platform` is `iphoneos`, `signed` is `false`, and the commit matches the workflow run.
