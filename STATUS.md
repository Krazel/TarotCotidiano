...
Fase actual: **`0.3 (1)` procesada y disponible mediante TestFlight Internal Only en iOS 16**. App Store Connect la muestra `En pruebas`, asignada al grupo interno `Testers` con dos invitaciones; todavía no registra una instalación de esta build. `0.2.2 (1)` queda como build anterior instalada. La `0.3` conserva el Home aprobado de A-042/V-058–V-063, integra la barra translúcida V-064, sustituye la tirada abierta por `Sí o no` bajo A-045/V-065–V-069 y simplifica Learn bajo A-046/V-070–V-071.
...
A-023 hace obligatoria la skill `ios-app-launch` para lanzamiento, StoreKit, privacidad, soporte, firma y assets de tienda. A-047 autorizó la subida de `0.3 (1)` restringida a TestFlight Internal Only; siguen sin autorización StoreKit, testers externos, review o publicación. Slug legal: `tarot-deck`; Support y Privacy responden en `https://krazel.github.io/tarot-deck/`.
...
Propietaria activa de implementación: **ninguna**. A-044 está integrada: la barra `Leer / Aprender / Cartas` usa blur oscuro fino, tinte grafito translúcido y un hairline superior tenue, con una única apariencia estable para standard y scroll-edge. A-045 está integrada: sustituye la tirada abierta por la tirada documentada `A favor / En contra / Resultado`; V-065–V-069 se crearon y registraron antes de la UI. A-046 queda cerrada bajo V-070/V-071: Learn agrupa siete tutoriales equivalentes y cada artículo conserva solo tres secciones prácticas, sin `Begin Here` ni disclaimers repetitivos. Los seis validadores, las 24 pruebas, el archive Release firmado y la subida pasan para `0.3 (1)`.
...
- Rama: `main`; `c4273f71729f3507ac8745cfea5cf4d81dc10bda` es la fuente exacta de TestFlight `0.3 (1)`. `e065562720acbde92075f6f94fae5b357067b974` permanece como fuente de TestFlight `0.2.2 (1)`, `aaaa83fe8ea6d4c32f5da23d7ea75da50361ced4` como fuente de `0.2.1 (1)` y `602b9d1c235082c1533bd83036acc7b35cea7f88` como fuente de la IPA Local-QA `0.2 (1)`.
...
2. **Cerrado:** `0.3 (1)` ejecuta la UI real en Release, usa el bundle definitivo, iOS 16 y privacidad; quedó firmada, subida, procesada y asignada como TestFlight Internal Only. App Store Connect la muestra `En pruebas` en `Testers`; todavía no registra instalaciones de esta build.
...
6. Settings S09.1 está integrado. A-047 cubre únicamente la subida ya ejecutada de `0.3 (1)` a TestFlight Internal Only. StoreKit, productos, testers externos, review y publicación siguen sin autorización. También siguen pendientes capturas finales y derechos de distribución pública.
...
Siguiente punto exacto: instalar y probar `0.3 (1)` desde TestFlight en el iPhone del propietario, comparando el selector/Home contra V-058–V-066, la mesa sí/no contra V-067–V-069, Learn contra V-070/V-071 y la barra inferior contra V-064. Una nueva build o subida necesita autorización expresa en ese turno. En paralelo, continuar la sustitución de las 78 imágenes antes de distribución externa o pública.
