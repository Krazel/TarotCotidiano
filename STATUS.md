...
A-057 autoriza en el turno actual subir `0.7.2 (1)` exclusivamente a TestFlight Internal Only. La puerta previa pasa: acceso global GitHub OK, entorno protegido `app-store-production` limitado a `main`, cuatro secretos requeridos presentes por nombre, versión/build nuevos, inventario de privacidad actualizado y gates locales verdes. La subida sigue pendiente hasta registrar commit, run, procesado y asignación interna.
