...
A-057 autoriza en el turno actual subir `0.7.2` exclusivamente a TestFlight Internal Only. El run `31612704651` sobre `2577cf7` se detuvo antes de firma/subida por una igualdad de fechas demasiado estricta en una prueba. Tras hacerla determinista, el run `31613200717` sobre `bda94c6` confirmó las 44 pruebas y detectó dos errores de compilación de integración antes de producir archive. `/root` es la única propietaria del ajuste operativo A-058: corregir ambas puertas pre-upload y recompilar la misma app como `0.7.2 (2)`, conforme A-032. La subida sigue pendiente hasta registrar commit, nuevo run, procesado y asignación interna.
