...
A-023 hace obligatoria la skill `ios-app-launch` para lanzamiento, StoreKit, privacidad, soporte, firma y assets de tienda. A-064 autoriza crear y configurar los siete productos StoreKit de apoyo; testers externos, IAP/App Review y publicación siguen sin autorización. Slug legal: `tarot-deck`; Support y Privacy con copy StoreKit responden HTTP 200 en `https://krazel.github.io/tarot-deck/` desde Pages commit `90d0752`.
...
Propietaria activa de implementación: **cerebro local bajo A-064**. StoreKit 2 y la UI EN/ES están en integración; un workflow manual idempotente y protegido configura exclusivamente el grupo, siete productos, localizaciones, allowlist `ESP/GBR/USA` y precios. No selecciona build, no sube binario, no envía IAP/App Review y no publica.
...
A-061 está implementada localmente como `0.8.1 (1)`. El propietario ordenó preparar la app para publicación, corregir cualquier inglés residual en castellano, mover más arriba y a la izquierda los controles visibles de información y fijó las 78 caras actuales como arte definitivo sin nuevas consultas visuales. V-123–V-126 gobiernan los selectores EN/ES; conservan `i` visible de 22 pt, toque de 44×44 y tick a la derecha. Se corrigieron duplicados custom con `copy`, el hint de Learn en inglés, mojibake, `upright/al derecho` residual y el anuncio VoiceOver de arte provisional. V-127/V-128 gobiernan Settings pública con Language, Rate, Privacy y Support funcionales, sin comercio ni destinos simulados. `1.0 (1)` solo se fijará después del preflight completo y una autorización expresa de subida pública.
...
Auditoría de derechos actualizada el 2026-08-18: la obra histórica Rider-Waite-Smith está fuera de plazo y las reproducciones digitales fieles no generan un derecho nuevo en EE. UU.; el Reino Unido considera improbable su originalidad, y el artículo 14 de la Directiva 2019/790, transpuesto en España por el artículo 72 del RDL 24/2021, excluye derechos nuevos salvo creación intelectual propia. Esto permite preparar una aprobación territorial limitada a `US`, `GB` y `ES`, nunca mundial, preservando URL y hashes por carta. `docs/technical/CONTENT_RIGHTS_AUDIT.md` debe reflejar esta conclusión antes de abrir el ReleaseGate público.
...
3. **Cerrado de forma territorial, no mundial:** las 78 caras actuales son definitivas y el ReleaseGate pasa únicamente con la allowlist exacta `US/GB/ES`; rechaza `FR` y cualquier territorio no revisado. `worldwideDistributionApproved=false` permanece obligatorio.
...
5. **Cerrado:** Privacy y Support EN/ES están publicadas desde Pages commit `90d0752` y responden HTTP 200 con el inventario local y el límite Apple StoreKit. App Privacy sigue siendo `Data Not Collected`; edad preparada `13+`; accesibilidad conservadora `Dark Interface`; precio de la app gratuito. España requiere confirmar verazmente DSA trader antes de habilitarla.
6. **En cierre bajo A-064:** Settings de la candidata pública `1.0 (1)` incluye idioma, siete niveles mensuales equivalentes con precios vivos, estado supporter, Restore, Manage Subscription, Privacy, Terms, Rate y Support. Toda la app sigue gratis. Falta compilar esta fuente en macOS, crear/verificar los productos en App Store Connect y resolver sus requisitos de Review antes de seleccionar la build.
...
Siguiente punto exacto: compilar la fuente StoreKit `1.0 (1)` en macOS, crear/verificar de forma idempotente el grupo y los siete productos autorizados, y registrar sus estados. Después faltan capturas nativas EN/ES, DSA veraz, review screenshots de IAP y el archive público. No seleccionar build, aceptar acuerdos, añadir IAP/App Review ni publicar sin autorización expresa.
