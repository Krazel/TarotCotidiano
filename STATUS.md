...
Fase actual: **candidata pública `1.0 (1)` preparada en App Store Connect y beta interna `1.0 (2)` disponible en TestFlight**. La build pública está procesada y seleccionada; el borrador completo espera la revisión de Apple de la información DSA antes del envío final.

La regla global A-022 queda activada por A-064/A-065: Settings incorpora siete niveles mensuales equivalentes, precios vivos de StoreKit, estado supporter, agradecimiento, restauración, gestión/cancelación, privacidad y términos. Toda la app sigue gratis y sin anuncios. Los productos y su grupo están incorporados al mismo borrador que la app.

A-023 hace obligatoria la skill `ios-app-launch` para lanzamiento, StoreKit, privacidad, soporte, firma y assets de tienda. A-065 autoriza preparar el envío conjunto de la app y las siete suscripciones. Slug legal: `tarot-deck`; Support y Privacy con copy StoreKit responden HTTP 200 en `https://krazel.github.io/tarot-deck/` desde Pages commit `90d0752`.
...
Propietaria activa de lanzamiento: **cerebro local bajo A-065**. StoreKit 2 y la UI EN/ES están integrados. App Store Connect tiene seleccionada la build pública `1.0 (1)`, ocho capturas, metadatos EN/ES, privacidad y soporte públicos, y la respuesta de derechos guardada. Las siete suscripciones y el grupo `Tarot Deck Support` están `Listo para revisión`, con capturas privadas, precios locales Apple y disponibilidad en los 175 países o regiones actuales más futuros. La app, siete productos y el grupo forman un único borrador de nueve elementos. La verificación DSA del comerciante figura `En revisión`; Apple mantiene desactivado el envío final hasta completar esa revisión. No se ha pulsado `Enviar a revisión` ni publicado.
...
A-061 quedó integrada en la candidata pública. El propietario ordenó preparar la app para publicación, corregir cualquier inglés residual en castellano, mover más arriba y a la izquierda los controles visibles de información y fijó las 78 caras actuales como arte definitivo sin nuevas consultas visuales. V-123–V-126 gobiernan los selectores EN/ES; conservan `i` visible de 22 pt, toque de 44×44 y tick a la derecha. Se corrigieron duplicados custom con `copy`, el hint de Learn en inglés, mojibake, `upright/al derecho` residual y el anuncio VoiceOver de arte provisional.

A-062 queda integrada y compilada en `1.0 (1)`: V-129/V-130 sustituyen la cápsula flotante por la misma zona estable que muestra las instrucciones normales. El modelo emite y consume una única petición por tirada nueva o reiniciada; restaurar, rotar o regresar desde Learn no la repite. V-116/V-117 siguen gobernando landscape y el mazo se mantiene como columna física derecha.

A-063 fijó formalmente la candidata pública `1.0 (1)`. El archive público está firmado, procesado y seleccionado; la ficha bilingüe, ocho capturas reales, páginas públicas y cumplimiento técnico están completados. App Store Connect mantiene un único borrador de nueve elementos preparado bajo A-065.

Las 78 caras quedan `visualFinal=true` por decisión explícita del propietario. App Store Connect tiene guardada la respuesta de Content Rights y la app está configurada para los 175 países o regiones actuales.
...
A-039 fija minimización estricta de datos e información pública. R-027 queda actualizada por A-064/R-037: la app no tiene cuentas, tracking, anuncios, analítica, permisos ni SDK de terceros; usa Apple StoreKit solo para precios, compra, verificación y restauración de apoyo. Idioma, selección, sesión/continuidad, favoritos, tiradas personalizadas y borrador permanecen locales. `PrivacyInfo.xcprivacy` declara `UserDefaults/CA92.1`; App Store Privacy `Data Not Collected` sigue siendo exacto para el desarrollador. Privacy/Support StoreKit están publicadas desde `90d0752`. App Store Connect muestra condición trader; P-006 exige confirmarla verazmente antes de habilitar España.
...
- SwiftUI para iPhone sigue aprobado. El núcleo y el flujo MVP completo compilan con Xcode 26.6 para `iphoneos` arm64; las ocho capturas reales aprobadas están cargadas en la ficha pública.
...
- El Settings histórico sin StoreKit quedó sustituido por A-064: la candidata `1.0 (1)` inyecta un único `SupporterStore`, muestra siete precios vivos, verifica entitlements y ofrece restauración/gestión/Privacy/Terms sin alterar la lectura ni bloquear funciones.
...
3. **Cerrado:** las 78 caras actuales son definitivas, su procedencia permanece registrada y Content Rights está guardado en App Store Connect.
4. **Cerrado:** Apple Developer contiene el App ID explícito `com.krazel.tarotdeck` y App Store Connect contiene la ficha iOS Apple ID `6800144105`, SKU permanente `tarot-deck-ios`, English (U.S.) principal, Español (España) adicional, nombres/subtítulos/descripciones/keywords bilingües, categorías Lifestyle/Reference y acceso ilimitado. La ficha no requiere login, usa publicación manual y tiene seleccionada la build pública `1.0 (1)`.
5. **Cerrado:** Privacy y Support EN/ES están publicadas desde Pages commit `90d0752` y responden HTTP 200 con el inventario local y el límite Apple StoreKit. App Privacy es `Data Not Collected`; edad `13+`; accesibilidad conservadora `Dark Interface`; precio de la app gratuito.
6. **Cerrado:** Settings incluye idioma, siete niveles mensuales equivalentes con precios vivos, estado supporter, Restore, Manage Subscription, Privacy, Terms, Rate y Support. Los siete productos y su grupo están listos y adjuntos al borrador.
7. **Pendiente externo:** Apple revisa la información DSA del comerciante. El botón `Enviar a revisión` permanece desactivado hasta que Apple complete esa revisión.
...
Siguiente punto exacto: esperar a que Apple cambie DSA de `En revisión`, repetir el preflight del borrador de nueve elementos y solicitar confirmación inmediata antes de pulsar `Enviar a revisión`. La publicación será manual incluso después de la aprobación.
